...
static volatile uint8_t mpu_who_am_i = 0;
...
#define MPU6500_MOT_DET_CTRL 0x69 
...
void MPU6050_Sensor_Init(void) {
  ...
  if (mpu_who_am_i != 0) {
    ...
    // 3. 配置加速計 (MPU6500 WoM 需要 DHPF 與特定的平均數設定)
    data = 0x01; // 5Hz DHPF
    HAL_I2C_Mem_Write(&hi2c1, mpu_active_addr, MPU6050_ACCEL_CONFIG, 1, &data, 1, 100);
    data = 0x08; // 4 samples average
    HAL_I2C_Mem_Write(&hi2c1, mpu_active_addr, MPU6500_ACCEL_CONFIG2, 1, &data, 1, 100);
    
    // 4. 配置中斷接腳
    data = 0x20; // Latch until read
    HAL_I2C_Mem_Write(&hi2c1, mpu_active_addr, MPU6050_INT_PIN_CFG, 1, &data, 1, 100);
    
    // 將 MOT_THR 恢復為 30，減少雜訊干擾
    data = 30;   
    HAL_I2C_Mem_Write(&hi2c1, mpu_active_addr, MPU6500_MOT_THR, 1, &data, 1, 100);
    
    data = 0xC0; // 啟用 ACCEL_INTEL_EN 與 ACCEL_INTEL_MODE
    HAL_I2C_Mem_Write(&hi2c1, mpu_active_addr, MPU6500_MOT_DET_CTRL, 1, &data, 1, 100);
...
void OLED_Display_Init(void) {
  ...
  if (mpu_init_status == 1) OLED_Put8x16Str(0, 2, "MPU: 6500 WoM");
...
void MPU6050_Read_Task(void) {
  ...
  for(;;) {
    wait_status = osSemaphoreAcquire(binSemMpuIntHandle, 250);
    (void)wait_status; // 消除 unused 警告
    
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);
    HAL_I2C_Mem_Read(&hi2c1, mpu_active_addr, MPU6050_INT_STATUS, 1, &int_status, 1, 10);
    
    if (HAL_I2C_Mem_Read(&hi2c1, mpu_active_addr, MPU6050_ACCEL_XOUT_H, 1, mpu_data, 14, 20) == HAL_OK) {
      if (!calibrate_flag) {
